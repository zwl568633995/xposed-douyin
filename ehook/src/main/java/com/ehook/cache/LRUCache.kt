...
object LRUCache {
    ...
    private const val ROOT_CACHE_DIR = "ehook"
    ...
    fun setup(memoryCacheSize: Long = 0, diskCacheSize: Long = 0) {
        ...
        diskCache = when (diskCacheSize == 0L) {
            true -> LRUDiskCache(
                getCacheDir(),
                diskCacheSize
            )
            false -> LRUDiskCache(getCacheDir())
        }
    }
    ...
    fun cachePath(dir: String, key: String, params: Map<String, Any>? = null, md5: Boolean = false): String =
        getCacheDir(dir) + File.separator + realKey(
            key,
            params,
            md5
        )
